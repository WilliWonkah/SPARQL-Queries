...
# Voraussetzung dafür: Ordner outputs/cluster_logs existiert im Projektverzeichnis.
#SBATCH --job-name=hf_download
#SBATCH --output=outputs/cluster_logs/download_%j.out
#SBATCH --error=outputs/cluster_logs/download_%j.err
...
if [ -f "$ENV_FILE" ]; then
    ...
    # Lade nur HF_TOKEN aus der .env (ignoriere Kommentare und leere Zeilen)
...
echo ""
echo "=== Schritt 2: Modelle herunterladen ==="
...
download_model "Qwen/Qwen2.5-Coder-7B-Instruct"
download_model "Qwen/Qwen2.5-7B-Instruct"
download_model "Qwen/Qwen2.5-32B-Instruct"
